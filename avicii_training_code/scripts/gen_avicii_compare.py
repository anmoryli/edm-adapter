"""Generate Avicii-style 2min tracks: baseline + LoRA for comparison."""
...
PROMPT = "progressive house, 128 BPM, piano chords, uplifting euphoric melody, anthemic synth lead, clean polished production, emotional, energetic drop, sidechain bass, supersaw chords, festival EDM, Swedish house style"
...
def install_control_conditioner(pipe, manifest_path):
    ...
    if not payload_path.exists():
        print("  [warn] No control_conditioner.pt found, skipping control conditioner")
        return False
...
def generate_baseline(pipe):
    ...
    save_path = OUTPUT_DIR / f"baseline_2min_{stamp}.wav"
    ...
    print(f"  Baseline saved: {output_paths}")
    return output_paths
...
def generate_lora(pipe):
    ...
    for spec in manifest.get("adapters", []):
        ...
        if not weight_path.exists():
            print(f"  [warn] Missing adapter: {weight_path}")
            continue
    ...
    # Install control conditioner
    install_control_conditioner(pipe, manifest_path)

    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    save_path = OUTPUT_DIR / f"lora_2min_{stamp}.wav"
    ...
    print(f"  LoRA saved: {output_paths}")
    return output_paths
...
def main():
    ...
    # Generate baseline first (no LoRA)
    baseline_paths = generate_baseline(pipe)

    # Reload model clean for LoRA generation
    ...
    lora_paths = generate_lora(pipe)

    print("\n=== Done ===")
    print(f"Baseline: {baseline_paths}")
    print(f"LoRA:     {lora_paths}")
