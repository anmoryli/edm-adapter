"""Train Avicii Style LoRA: single adapter for style transfer."""
...
def build_style_lora_config():
    """Single LoRA adapter config for style learning."""
    return {
        "r": 32,
        "lora_alpha": 16,
        "target_modules": [
...
def build_lightning_module_class():
    ...
    class AviciiStylePipeline(BasePipeline):
        ...
        def on_save_checkpoint(self, checkpoint):
            ...
            bundle_dir = Path(log_dir) / "checkpoints" / f"epoch={epoch}-step={step}_avicii_style"
...
def main() -> None:
    ...
    parser.add_argument("--logger-dir", default=str(PROJECT_ROOT / "outputs" / "avicii_style_lora" / "logs"))
    parser.add_argument("--output-dir", default=str(PROJECT_ROOT / "outputs" / "avicii_style_lora"))
    parser.add_argument("--exp-name", default="avicii_style")
    ...
    lora_config_path = runtime_dir / "style_lora.json"
